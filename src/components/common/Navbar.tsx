...
export default function Navbar() {
    ...
    const fetchLogo = async () => {
      try {
        ...
        if (response.ok) {
          ...
          if (data) {
            if (data.profileImage && typeof data.profileImage === 'string' && data.profileImage.trim()) {
              setLogoImage(data.profileImage.trim());
            }
  ...
  return (
    <>
      {/* Dynamic Keyframes for Premium animations */}
      ...
      >
        <div
          className={`mx-auto flex max-w-7xl items-center justify-between transition-all duration-500 ease-out ${
            isScrolled
              ? 'bg-[#050816]/90 backdrop-blur-2xl rounded-full px-4 sm:px-8 py-2.5 sm:py-3 border border-white/[0.08] shadow-[0_12px_40px_-10px_rgba(0,0,0,0.5)]'
        ...
        >
          ...
          <a
            ...
            className="flex items-center space-x-2.5 group cursor-pointer flex-shrink-0 z-10"
          >
                ...
                {navLinks.map((link, idx) => {
                  ...
                  return (
                    <motion.a
                      ...
                      href={link.href}
                      onClick={(e) => handleLinkClick(e, targetId)}
                      className={`font-sans text-xl font-bold uppercase tracking-wider transition-colors duration-200 cursor-pointer ${
                        isActive ? 'text-accent-cyan font-extrabold' : 'text-slate-300 hover:text-white'
                      }`}
